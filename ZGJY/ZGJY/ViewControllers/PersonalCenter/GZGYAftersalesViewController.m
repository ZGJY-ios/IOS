...
{
    self.segView = [[GZGYSegView alloc]initWithFrame:CGRectMake(0, [GZGApplicationTool navBarAndStatusBarSize], KScreenWigth, [GZGApplicationTool control_height:95]) NameArray:_nameArray];
...
- (void)addTableViewToScrollView:(UIScrollView *)scrollView count:(NSUInteger)pageCount frame:(CGRect)frame {
    for (int i = 0; i < pageCount; i++) {
        UITableView *tableView = [[UITableView alloc]initWithFrame:CGRectMake(KScreenWigth * i, 0 , KScreenWigth, KScreenHeight - self.segView.frame.size.height - 64)];
